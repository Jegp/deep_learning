...
let replicate = dl.layers.replicate 3 dl.nn.identity 1
...
entry replicate_backward_e input w b =
  let ws = ((w, b), (w, b))
  let (cache, output) = replicate.forward true ws input
  let (e', _) = replicate.backward false updater ws cache output
  in e'
